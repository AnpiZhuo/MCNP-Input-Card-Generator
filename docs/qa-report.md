...
> ⚠️ **历史报告更正注记（2026-09-10，文档债 TD-29 修复）**：本报告写作时（2026-08-24/25）文中引用的 `hexCenter` 权威公式为**旧式** `x=i·p·√3/2, y=j·p+(i%2)·p/2`（见 §15 项 5 一行），该式**与 2026-08-25 交叉验证后的权威公式相差 30° 旋转**。**权威公式 = `x=col*pitch+row*pitch/2, y=row*pitch*√3/2`**，原文见 `gui/src/utils/lattice.ts:130-137` ↔ `app/lattice.py:604-616`。本报告的历史复跑结论**不改**（当时门禁确实全绿），仅公式文字以权威为准。
