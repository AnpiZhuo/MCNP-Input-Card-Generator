...
def validate_all(
    ...
    # ----- 计数卡 (F1~F8) -----
    if tally.f1_enabled and not tally.f1_surface.strip():
        errors.append("计数：F1 的曲面号不能为空")
    if tally.f2_enabled and not tally.f2_surface.strip():
        errors.append("计数：F2 的曲面号不能为空")
    if tally.f4_enabled and not tally.f4_cell.strip():
        errors.append("计数：F4 的栅元号不能为空")
    if tally.f5_enabled:
        if not all([tally.f5_x.strip(), tally.f5_y.strip(), tally.f5_z.strip()]):
            errors.append("计数：F5 的 X/Y/Z 坐标不能为空")
    if tally.f6_enabled and not tally.f6_cell.strip():
        errors.append("计数：F6 的栅元号不能为空")
    if tally.f7_enabled and not tally.f7_cell.strip():
        errors.append("计数：F7 的栅元号不能为空")
    if tally.f8_enabled and not tally.f8_cell.strip():
        errors.append("计数：F8 的栅元号不能为空")
    ...
    # 计数卡引用的栅元/曲面号检查
    def _check_cell_ref(tally_name: str, cell_str: str):
        if cells and cell_str.strip():
            for cid in cell_str.split():
                if cid.isdigit() and not any(c.number == int(cid) for c in cells):
                    errors.append(f"计数：{tally_name} 引用的栅元 {cid} 未定义")

    if tally.f4_enabled:
        _check_cell_ref("F4", tally.f4_cell)
    if tally.f6_enabled:
        _check_cell_ref("F6", tally.f6_cell)
    if tally.f7_enabled:
        _check_cell_ref("F7", tally.f7_cell)
    if tally.f8_enabled:
        _check_cell_ref("F8", tally.f8_cell)
