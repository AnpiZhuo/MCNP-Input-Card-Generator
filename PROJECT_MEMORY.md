...
- **✅ 用户裁决（接手时就 5 个悬置决策逐条拍板）**：
  ...
  4. **SI 类型 `V`（C810 非法）** → **用户裁决"本批修"，已修（TD-35 关闭）**：`app/generator/inp_generator.py:_build_multi_sisp_cards` 对 POS_VEC 原发 `SI{di}  V  <平坦值>`，而 **`V` 不是合法 SI 字母**（C810 只认 H/L/A/S）。已改为**一律发合法 `L`**（POS_VEC 本质是位置向量**列表**）。解析侧 `_LEGACY_SI_LETTERS = ("V",)` **予以保留**（旧输入卡往返不容崩，抽样侧仍报错）。**实测风险小于预期**：全量 pytest **875 passed**，R1 不动点 / R4 kitchen-sink 字节断言**未回归**；仅 `tests/unit/test_generator_multi_source.py:75` 一处断言锁死旧 `SI1  V`，已随修（并加"新生成卡不得含 `V`"反向断言）。
  5. **是否先提交** → **先提交再验证**（已执行）。
